  ...
  useEffect(() => {
    ...
    const trigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // Fade overlay: start fading at 80%, fully opaque at 90%
        if (progress >= 0.80) {
          const fadeProgress = Math.min((progress - 0.80) / 0.10, 1);
          setFadeOpacity(fadeProgress);
        } else {
          setFadeOpacity(0);
        }

        // Transition to OS at 90%
        if (progress >= 0.90 && !isInsideOS) {
          setIsInsideOS(true);
        } else if (progress < 0.87 && isInsideOS) {
          setIsInsideOS(false);
        }
      },
    });
